<?php
    ...
    function createNewTextRecord(){
      ...
      if($id == 'list-item'){
        ...
        $listInsertSql = "INSERT INTO aediustable(page, pageContent) VALUES('$section', ' $listString')";
        insertRowResponse($listInsertSql);
      }else{
        $singleItem = array('id' => $id, 'value'=> $value);
        $textedNewList = json_encode(array($singleItem));
        $newInsertSql = "INSERT INTO aediustable(page, pageContent) VALUES('$section', '$textedNewList')";
    ...
    function updateText($selectedText){
      ...
      elseif($id =='delete-list'){
        array_splice($detextedItem,$value,1);
      }else{
        for($i=0; $i < $length; $i++) { 
      ...
      $updatedRecord = "UPDATE aediustable SET pageContent = '$updatedString' where page = '$section'";
    ...
    function selectRow(){
      global $section;
      $sql_select = "SELECT * FROM aediustable WHERE page = '$section'";
